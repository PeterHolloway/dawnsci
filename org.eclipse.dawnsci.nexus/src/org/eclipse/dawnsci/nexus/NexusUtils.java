...
public class NexusUtils {
	...
	public static List<String> getAxisNames(String filePath, String nexusPath, int dimension) throws Exception {
		...
		NexusFile file = null;
		try {
			file = ServiceHolder.getNexusFileFactory().newNexusFile(filePath);
			file.openToRead();
			DataNode signal = file.getData(nexusPath);
			return getAxes(file, signal, dimension);
		} finally {
			if (file != null)
				file.close();
		}
	 ...
	 */
	public static List<String> getAxes(final NexusFile file, final DataNode signal, int dimension) throws Exception {
		...
		final String parentPath = file.getPath(signal).substring(0, file.getPath(signal).lastIndexOf("/"));
					...
					if ("axis".equals(attribute.getName())) {
						...
						if (iaxis < 0) { // We look for comma separated string
							...
							if (axesDims != null && axesCompatible(axesDims, shapeAxes, shapeData)) {
								for (int dim : axesDims) {
									if (dim == dimension) {
										axis = parentPath + "/" + dataNode.getDataset().getName() + ":" + dimension;
										break;
									}
								}
							}
						...
						if (iaxis == dimension) {
							...
							if ((dims.length == 1 && dims[0] == size) || dims.length != 1) {
								axis = parentPath + "/" + dataNode.getDataset().getName();
							}
